...
//            System.out.println("req method is " + exchange.getRequestMethod().toLowerCase());
            if (exchange.getRequestMethod().toLowerCase().equals("get") || exchange.getRequestMethod().toLowerCase().equals("post")){
                ...
                //call service
                Boolean response = assignmentDAO.addAssignment(dumbData.assignmentID, dumbData.AssignmentName, dumbData.ClassID, dumbData.DueDate, dumbData.DoDate);
                //check for non error
                if(response != null){
            ...
            if(!success){
                ...
                String jsonStr = gson.toJson("add assignment failed");
                respBody.write(jsonStr);
                respBody.flush();
                respBody.close();
            }
